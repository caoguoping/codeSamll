...
-- add by leo
function decodeFile(typename, decodeFunc, filename)
	local helper = PbcHelper
	local ret = {}
	local ok = decodeFunc( P, decode_message_cb , ret , typename, filename)
	if ok then
		return setmetatable(ret , default_table(typename))
	else
		printInfo(string.format("Can't decodeFile Error:%s", c._last_error(pb.P)))
		return false
	end
end
-- end by leo
