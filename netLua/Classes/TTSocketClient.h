#ifndef _TT_SOCKET_CLIENT_H_
#define _TT_SOCKET_CLIENT_H_

#include "MTCustomEventQueue.h"

//#if (PlatWhich == PlatWin)
#ifdef WIN32
#include <WinSock2.h>
#pragma comment(lib, "WS2_32.lib")
typedef int				socklen_t;
#else

#include <arpa/inet.h>		// for inet_**
#include <netdb.h>			// for gethost**
#include <netinet/in.h>		// for sockaddr_in
#include <sys/types.h>		// for socket
#include <sys/socket.h>		// for socket
#include <unistd.h>
#include <stdio.h>		    // for printf
#include <stdlib.h>			// for exit
#include <string.h>			// for bzero
#include <unistd.h>
typedef int				SOCKET;
#define INVALID_SOCKET	-1
#define SOCKET_ERROR	-1
#endif


#define SEND_BUFFER_LENGTH	2048
#define SOCKET_TCP_BUFFER	4096	
//网络内核
struct TCP_Info
{
	unsigned char							cbDataKind;																			//数据类型
	unsigned char							cbCheckCode;																		//效验字段
	unsigned short							wPacketSize;																		//数据大小
};

//网络命令
struct TCP_Command
{
	unsigned short							wMainCmdID;																			//主命令码
	unsigned short							wSubCmdID;																			//子命令码
};

//网络包头
struct TCP_Head
{
	TCP_Info								TCPInfo;																			//基础结构
	TCP_Command								CommandInfo;																		//命令信息
};

//发送映射
const unsigned char g_SendByteMap[256] =
{
	
};


//接收映射
const unsigned char g_RecvByteMap[256] =
{
	
};

class TTSocketClient
{
public:
	TTSocketClient();
	~TTSocketClient();
	static TTSocketClient* getInstance();
	void startSocketLogin(const char *ip, unsigned short port);
	unsigned short CrevasseBuffer(unsigned char pcbDataBuffer[], unsigned short wDataSize);									//解密数据
	unsigned char MapRecvByte(unsigned char const cbData);
	bool ConnectIPv4(const char *ip, unsigned short port, unsigned char bSocketType);
	int Send(const char* buf, int len, unsigned char bSocketType, int flags = 0);
//	int Recv(char* buf, int len, int flags, unsigned char bSocketType);
	int closeMySocket(unsigned char bSocketType);
	bool recvDateLogin(void);

public:
	bool					m_isOpenSocketLogin;
	bool					m_isOpenSocketGame;
	SOCKET					mSocketLogin;
	SOCKET			        mSocketGame;
	unsigned char			m_cbRecvBufLogin[SOCKET_TCP_BUFFER];		//接收缓冲
	unsigned char			m_cbRecvBufGame[SOCKET_TCP_BUFFER];		//接收缓冲
	time_t lastRcvTimeLogin;    //最后一次收到报文的时间
	time_t lastRcvTimeGame;    //最后一次收到报文的时间

	char			m_cbRcvPushLogin[SOCKET_TCP_BUFFER];		//接收缓冲
	char			m_cbRcvPushGame[SOCKET_TCP_BUFFER];		//接收缓冲

};

#endif 
