...
// 				memcpy(pData, m_cbRecvBufLogin, wPacketSize);
				memcpy(m_cbRcvPushLogin, m_cbRecvBufLogin, wPacketSize);
				MTCustomEventQueue::getInstance()->pushCustomEvent("rcvDataLogin", m_cbRcvPushLogin);
				for (int i = 0; i < 10; i++)
				{
				//	log("TT %02x ", (unsigned char)m_cbRcvPushLogin[i]);
				}
			//	log("new pData %p\n\n\n", pData);
